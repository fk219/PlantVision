...
const Navbar = () => {
  ...
  return (
    <nav ref={navRef} className="bg-white/90 backdrop-blur-lg border-b border-green-100 sticky top-0 z-50 shadow-sm h-[6vw]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div>
              <img src={logo} alt="Logo"  className="w-15 h-15 sm:w-[6rem] sm:h-[6rem]"/>
            </div>
          </div>
            ...
            <div className="flex gap-2 lg:gap-4">
              {navLinks.map(({ id, path, label, icon: Icon }) => (
                <NavLink
                  ...
                  to={path}
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-4 py-3 rounded-lg font-medium text-sm lg:text-base transition-all duration-300 ${
                      isActive
